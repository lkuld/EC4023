...
***I have loaded the following packages into this tutorial: EC4023, stringr, dplyr, and ggplot2.***

`ggplot2` implements the Grammar of Graphics^[Leland Wilkinson (1999), The Grammar of Graphics.]—hence the “gg”. It provides a systematic way to build both common and novel plots (histograms, scatterplots, maps, and more).

The core idea is to build a plot by adding layers:

  - declare a dataset and aesthetics (which variables map to axes, colour, size);

  - add a geometric layer (e.g., geom_point() for a scatterplot, geom_histogram() for a histogram);

  - refine with scales, labels, guides, and themes.

The dataset `bairoch` contains the population size of European cities based on "La Population Des Villes Europeennes" (Bairoch, Batou, Chevre 1988). 

Press run below to show the first observations (rows). 

**Task:** Change the code to display only observations from Ireland.
...
Limerick <- bairoch |> filter(city=="Limerick" & !is.na(population)) 
...
# create data subset
Limerick <- bairoch |> filter(city %in% c("Limerick", "Cork", "Galway") & !is.na(population)) 
